...
<script type="text/javascript">
	...
	//去修改页面
	function toBrandtUpdatePage(id){
		window.location.href = "/toBrandtUpdatePage.do?id="+id;
	}
</script>
</head>
<body>
	<div class="box-positon">
		...
		<form class="ropt">
			<input class="add" type="button" value="添加"
				onclick="javascript:window.location.href='toBrandAdd.do'" />
		</form>
		<div class="clear"></div>
	</div>
	<div class="body-box">
		<form action="selectAllBrand.do" method="post"
			style="padding-top: 5px;">
			品牌名称: <input type="text" name="name" value="${brand.name}" /> <select
				name="isDisplay">
				<option value="2">请选择</option>
				<option value="1" <c:if test="${brand.isDisplay==1 }">selected="selected"</c:if> >是</option>
				<option value="0" <c:if test="${brand.isDisplay==0 }">selected="selected"</c:if> >否</option>
			...
			<tbody class="pn-ltbody">
				<c:forEach items="${blist }" var="b">
					<tr bgcolor="#ffffff" onmouseout="this.bgColor='#ffffff'"
						...
						<td align="center"><c:if test="${b.isDisplay==1 }">是</c:if> <c:if
								test="${b.isDisplay==0 }">否</c:if></td>
						<td align="center"><a class="pn-opt" href="javascript:;"
							onclick="toBrandtUpdatePage(${b.id })">修改</a> | <a
							class="pn-opt"
							onclick="if(confirm('您确定删除吗？')) {window.location.href='delBrand.do?id=${b.id }'}"
