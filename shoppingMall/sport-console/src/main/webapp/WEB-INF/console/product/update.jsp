...
<body>
	...
	<div class="body-box" style="float: right">
		<form id="jvForm" action="productUpdate.do" method="post"
				...
				<tbody id="tab_1">
					<tr>
						...
						<td width="80%" class="pn-fcontent"><select name="typeId">
								...
								<option value="5" selected="selected">瑜伽垫</option>
					...
					<tr>
						...
						<td width="80%" class="pn-fcontent"><select name="brandId">
								...
								<c:forEach items="${blist }" var="b">
									<option value="${b.id }"
										<c:if test="${product.brandId==b.id }">selected="selected"</c:if>>${b.name }</option>
								</c:forEach>
						</select></td>
					</tr>
					<tr>
						...
						<td width="80%" class="pn-fcontent"><input type="text"
							value="${product.weight }" class="required" name="weight"
							maxlength="10" />KG</td>
					</tr>
					<tr>
						<td width="20%" class="pn-flabel pn-flabel-h"><span
						...
						<td width="80%" class="pn-fcontent"><input type="checkbox"
							value="9" name="colorss"
							
							<c:forEach items="${colors }" var="c">
								<input type="checkbox" value="${c.id }" name="colorss" <c:forEach items="${product.colorss }" var="cs"><c:if test="${cs==c.id }">checked="checked"</c:if></c:forEach>/>${c.name }
							</c:forEach>
						</td>
					</tr>
					<tr>
						...
						<td width="80%" class="pn-fcontent"><input type="checkbox"
							name="sizess" value="S"
							<c:forEach items="${product.sizess }" var="size"><c:if test="${size=='S' }">checked</c:if></c:forEach> />S
							<input type="checkbox" name="sizess" value="M"
							<c:forEach items="${product.sizess }" var="size"><c:if test="${size=='M' }">checked</c:if></c:forEach> />M
							<input type="checkbox" name="sizess" value="L"
							<c:forEach items="${product.sizess }" var="size"><c:if test="${size=='L' }">checked</c:if></c:forEach> />L
							<input type="checkbox" name="sizess" value="XL"
							<c:forEach items="${product.sizess }" var="size"><c:if test="${size=='XL' }">checked</c:if></c:forEach> />XL
							<input type="checkbox" name="sizess" value="XXL"
							<c:forEach items="${product.sizess }" var="size"><c:if test="${size=='XXL' }">checked</c:if></c:forEach> />XXL
						</td>
					</tr>
					<tr>
						...
						<td width="80%" class="pn-fcontent"><input type="checkbox"
							name="isNew" value="1"
							<c:if test="${product.isNew==1 }">checked</c:if> />新品 <input
							type="checkbox" name="isCommend" value="1"
							<c:if test="${product.isCommend==1 }">checked</c:if> />推荐 <input
							type="checkbox" name="isHot" value="1"
							<c:if test="${product.isHot==1 }">checked</c:if> />热卖</td>
					</tr>
				</tbody>
				<tbody id="tab_2" style="display: none">
					...
					<tr>
						...
						<td width="80%" class="pn-fcontent"><c:forEach
								items="${product.imgUrls }" var="img">
								<img width="100" height="100" src="${img }" />
								<input type="hidden" name="imgUrls1" value="${img }" />

								<!-- <a href="javascript:;" class="pn-opt"
									onclick="jQuery(this).parents(\'tr\').remove()">删除</a> -->
							</c:forEach></td>
				...
				<tbody id="tab_4" style="display: none">
					<tr>
						<td><textarea rows="20" cols="180" id="productList"
								name="packageList">${product.packageList}</textarea></td>
					</tr>
				</tbody>
				<tbody>
					<tr>
			...
			<input type="hidden" name="id" id=id value="${product.id}" /> <input
				...
				type="hidden" name="size" id="size" value="${size }" />
		</form>
	</div>
</body>
</html>